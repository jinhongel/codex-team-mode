...
- **Executor（执行者）· Luna xHigh · 可写**：在范围、业务语义、接口、数据模型、状态流程、验收标准和安全边界已经明确后完成实现、修复和测试。xHigh 比 High 多留一档质量余量，同时把 Max 留给真正最难、质量优先的极端任务，而不是让所有有边界实现默认跑到最高档。
...
- 架构、产品与业务语义、接口、数据模型、状态流程、范围、安全边界和最终验收仍由主线程负责。
